...
import sprout.jooq.generate.tables.records.VerificationCodeRecord;
...
import static sprout.jooq.generate.tables.VerificationCode.VERIFICATION_CODE;
...
public class AccountApplicationService {
    ...
    public TenantId createTenant(String adminName, String adminPassword, String tenantName, String captcha) {
        ...
        Validate.isTrue(Instant.now().isBefore(record.getExpireAt().toInstant()), "error.verification.code.expired");
    ...
    public void enableTenant(String tenantId, boolean enabled) {
        Tenant tenant = tenantRepository.findByTenantId(new TenantId(UUID.fromString(tenantId).toString()))
    ...
    public void sendCaptcha(String username) {
        ...
        if (record == null) {
            jooq.insertInto(VERIFICATION_CODE)
                    ...
                    .set(VERIFICATION_CODE.EXPIRE_AT, Timestamp.from(expireAt))
                    .execute();
        } else {
            record.setCaptcha(captcha.toString());
            record.setExpireAt(Timestamp.from(expireAt));
